...
const childrenMenus = computed(() => {
    ...
    return constantRouteMap.concat(childrenMenus)
})
...
function setVisibleNumber() {
    const width = (document.body.getBoundingClientRect().width / (3 * 85)).toString()
    visibleNumber.value = parseInt(width)
}
...
function handleSelect(key, keyPath) {
  ...
  if (isHttp(key)) {
    ...
    // 没有子路由路径内部打开
    const routeMenu: any = childrenMenus.value.find(item => item.path === key)
